...
# CENTRAL UI INPUT ARBITER (DETERMINISTIC EVENT COMPOSITOR)
# ---------------------------------------------------------
...
func _ready():
	...
	print("[INPUT ARBITER] Online. Enforcing exclusive STOP invariant and state-driven input zoning.")
	...
	match state:
		...
		UIState.MODAL_ACTIVE:
			...
			if is_instance_valid(_modal_active_panel) and _modal_active_panel != panel:
				print("[INPUT ARBITER] Exclusivity enforced. Stripping STOP from: ", _modal_active_panel.name)
...
func begin_transition(panel: Control):
	...
	print("[INPUT ARBITER] Transitional Lock ENGAGED. Suppressing input race conditions.")
...
func end_transition(panel: Control, final_state: int):
	...
	print("[INPUT ARBITER] Transition resolved. Remaining active locks: ", _active_transitions_count)

func _recalculate_global_locks():
