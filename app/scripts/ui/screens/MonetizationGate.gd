...
func setup_universe_unlock(universe_id: String):
	...
	if subtitle_label: subtitle_label.text = "Permanently add this universe to your collection for $2.99."
	if btn_buy: btn_buy.text = "UNLOCK ($2.99)"
...
func setup_directors_pass():
	...
	if btn_buy: btn_buy.text = "REMOVE ADS ($7.99)"

func _ready():
	if btn_buy: btn_buy.pressed.connect(_on_buy)
	if btn_cancel: btn_cancel.pressed.connect(func():
