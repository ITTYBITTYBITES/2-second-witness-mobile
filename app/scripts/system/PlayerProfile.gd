...
func generate_insights() -> Array[String]:
	var narrator = Engine.get_main_loop().root.get_node_or_null("MirrorNarrator") if Engine.get_main_loop() else null
	if narrator and narrator.has_method("get_insights"):
		return narrator.get_insights(self)
	return ["Awaiting observation data to form a reflection..."]
