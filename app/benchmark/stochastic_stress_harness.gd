...
func _init():
	...
	for i in range(test_universes.size()):
		...
		print("  [Cycle ", i+1, "] Injecting Universe ID: '", u_id, "'")
		if u_id.begins_with("invalid") or u_id.begins_with("corrupt"):
			...
			print("    ✅ Valid universe '", u_id, "' streamed cleanly.")
	...
	for i in range(5):
		if kernel.is_ui_blocking():
			print("    [Pointer Click ", i+1, "] Suppressed by active transition lock.")
		else:
	...
	for i in range(3):
		print("    [Billing Callback ", i+1, "] Firing _on_purchase_success('", target_sku, "')")
