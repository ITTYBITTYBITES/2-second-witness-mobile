...
func _refresh() -> void:
	...
	vb.add_child(_create_setting_row_toggle("Dark Mode", "theme_mode", SettingsService.get_value("theme_mode", "dark") == "dark", _on_theme_toggle))
	vb.add_child(_create_setting_row_toggle("Reduced Motion", "reduced_motion", SettingsService.get_value("reduced_motion", false), _on_generic_toggle))
	vb.add_child(_create_setting_row_slider("Font Scale", "font_scale", SettingsService.get_value("font_scale", 1.0), 0.8, 1.4, 0.1))
	
	# Audio Section
	vb.add_child(_create_section_header("Audio"))
	vb.add_child(_create_setting_row_slider("Master Volume", "volume_master", SettingsService.get_value("volume_master", 1.0), 0.0, 1.0, 0.1))
	vb.add_child(_create_setting_row_slider("BGM Volume", "volume_bgm", SettingsService.get_value("volume_bgm", 0.7), 0.0, 1.0, 0.1))
	vb.add_child(_create_setting_row_slider("SFX Volume", "volume_sfx", SettingsService.get_value("volume_sfx", 0.9), 0.0, 1.0, 0.1))
	vb.add_child(_create_setting_row_toggle("Haptics", "haptics_enabled", SettingsService.get_value("haptics_enabled", true), _on_generic_toggle))
	
	# Accessibility Section
	vb.add_child(_create_section_header("Accessibility"))
	vb.add_child(_create_setting_row_toggle("High Contrast", "high_contrast", SettingsService.get_value("high_contrast", false), _on_generic_toggle))
	vb.add_child(_create_setting_row_toggle("Screen Reader Hints", "accessibility_screen_reader_hints", SettingsService.get_value("accessibility_screen_reader_hints", false), _on_generic_toggle))
	vb.add_child(_create_setting_row_toggle("Reduce Motion (Accessibility)", "accessibility_reduce_motion", SettingsService.get_value("accessibility_reduce_motion", false), _on_generic_toggle))
	
	# Gameplay
	vb.add_child(_create_section_header("Gameplay"))
	vb.add_child(_create_setting_row_toggle("Show Tutorials", "show_tutorials", SettingsService.get_value("show_tutorials", true), _on_generic_toggle))
	vb.add_child(_create_setting_row_toggle("Auto Play Next", "auto_play_next", SettingsService.get_value("auto_play_next", false), _on_generic_toggle))
	
	# Privacy
	vb.add_child(_create_section_header("Privacy & Data"))
	vb.add_child(_create_setting_row_toggle("Analytics", "analytics_enabled", SettingsService.get_value("analytics_enabled", true), _on_generic_toggle))
	vb.add_child(_create_setting_row_toggle("Crash Reporting", "crash_reporting", SettingsService.get_value("crash_reporting", true), _on_generic_toggle))
	
	# About
	vb.add_child(_create_section_header("About"))
	vb.add_child(_create_info_row("App Version", ConfigService.get_value("app_version", "2.0.0") if ConfigService else "2.0.0"))
...
func _create_setting_row_toggle(label: String, key: String, value: bool, callback: Callable) -> Control:
...
func _create_setting_row_slider(label: String, key: String, value: float, min_v: float, max_v: float, step: float) -> Control:
	...
	val_lbl.text = "%.1f" % value if max_v <= 1.5 else "%d%%" % int(value * 100) if max_v == 1.0 else "%.1f" % value
	var fmt_val: String = ""
...
func _create_info_row(label: String, value: String) -> Control:
	...
	var style := StyleBoxFlat.new()
	if ThemeService:
		style.bg_color = Color(ThemeService.tokens.get("surface", Color("#1E1E26")), 0.5)
...
func _on_theme_toggle(_key: String, is_dark: bool) -> void:
	...
	if ThemeService:
		ThemeService.set_theme_mode(ThemeService.ThemeMode.DARK if is_dark else ThemeService.ThemeMode.LIGHT)
	if AccessibilityService:
		AccessibilityService.vibrate(20)
