...
func _on_answer(chose_risk: bool):
	...
	if not chose_risk:
		...
		PlayerProfile.record_cognitive_event("decision_confidence", "science_lab", true, rt_ms) # True because it wasn't a "failure"
		SessionTracker.record_spike_result("risk_selection_safe", true)
		_eject()
	else:
		if risk_succeeds:
			feedback_label.text = "RISK REWARDED! SLINGSHOT INITIATED!"
			PlayerProfile.record_cognitive_event("decision_confidence", "science_lab", true, rt_ms)
			SessionTracker.record_spike_result("risk_selection_risk", true)
			_eject()
		else:
			print("[RISK SELECTION] Error. Resetting.")
			PlayerProfile.record_cognitive_event("decision_confidence", "science_lab", false, rt_ms)
